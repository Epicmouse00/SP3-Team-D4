	...
	, barStatus(0)
{
...

	{
		...
		staminaBar[0] = Create::Sprite2DObject("Stamina_Green",
			Vector3((thePlayerInfo->GetStaminaSpeed() - 0.3f) / 2 + 18, 208.f, 0.0f),// Note : replace 0.5 with stamina????
			Vector3((thePlayerInfo->GetStaminaSpeed() - 0.3f), 14.f, 0.0f));
		staminaBar[1] = Create::Sprite2DObject("Stamina_Amber",
			Vector3((thePlayerInfo->GetStaminaSpeed() - 0.3f) / 2 + 18, 208.f, 0.0f),
			Vector3((thePlayerInfo->GetStaminaSpeed() - 0.3f), 14.f, 0.0f));
...
bool UserInterface::Update(double dt)
{
	...
	case SC_PLAY: // This just checks for changes in UI* stuff while in play
	{
		...
		barStatus = 0;
		if (thePlayerInfo->GetStaminaSpeed() < 0.65f)
		{
			barStatus = 1;
			if (thePlayerInfo->GetStaminaSpeed() < 0.5f)
				barStatus = 2;
		}
		staminaBar[barStatus]->SetScale(Vector3((thePlayerInfo->GetStaminaSpeed()-0.3f) * 50,staminaBar[barStatus]->GetScale().y,staminaBar[barStatus]->GetScale().z));
